...
//        echo '<br />';
    ...
    private function readFont()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            $underlineType = ord(substr($recordData, 10, 1));
    ...
    private function readXf()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            $horAlign = (0x07 & ord(substr($recordData, 6, 1))) >> 0;
            switch ($horAlign) {
                case 0:
            ...
            $wrapText = (0x08 & ord(substr($recordData, 6, 1))) >> 3;
            switch ($wrapText) {
                case 0:
            ...
            $vertAlign = (0x70 & ord(substr($recordData, 6, 1))) >> 4;
            switch ($vertAlign) {
                case 0:
            ...
            if ($this->version == self::XLS_BIFF8) {
                ...
                $angle = ord(substr($recordData, 7, 1));
                $rotation = 0;
                if ($angle <= 90) {
                    $rotation = $angle;
                } elseif ($angle <= 180) {
                ...
                $indent = (0x0F & ord(substr($recordData, 8, 1))) >> 0;
                $objStyle->getAlignment()->setIndent($indent);

                // bit: 4; mask: 0x10; 1 = shrink content to fit into cell
                $shrinkToFit = (0x10 & ord(substr($recordData, 8, 1))) >> 4;
                switch ($shrinkToFit) {
                    case 0:
                ...
                $orientationAndFlags = ord(substr($recordData, 7, 1));
            ...
            while ($offset < $length) {
                ...
                switch ($extType) {
                    case 4:        // fill start color
                        ...
                        if ($xclfType == 2) {
                            $rgb = sprintf('%02X%02X%02X', ord(substr($xclrValue, 0, 1)), ord(substr($xclrValue, 1, 1)), ord(substr($xclrValue, 2, 1)));
    ...
    private function readStyle()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            if ($isBuiltIn) {
                ...
                $builtInId = ord(substr($recordData, 2, 1));
    ...
    private function readSheet()
    {
        ...
        switch (ord(substr($recordData, 4, 1))) {
            case 0x00:
        ...
        $sheetType = ord(substr($recordData, 5, 1));
    ...
    private function readDefinedName()
    {
        ...
        if ($this->version == self::XLS_BIFF8) {
            ...
            $nlen = ord(substr($recordData, 3, 1));
    ...
    private function readSst()
    {
        ...
        for ($i = 0; $i < $nm; ++$i) {
            ...
            // option flags
            $optionFlags = ord(substr($recordData, $pos, 1));
            ++$pos;
            ...
            if ($pos + $len <= $limitpos) {
                ...
                while ($charsLeft > 0) {
                    ...
                    $option = ord(substr($recordData, $pos, 1));
                    ++$pos;
    ...
    private function readFormula()
    {
        ...
        $isPartOfSharedFormula = $isPartOfSharedFormula && ord(substr($formulaStructure, 2, 1)) == 0x01;
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            ...
            if ((ord(substr($recordData, 6, 1)) == 0) && (ord(substr($recordData, 12, 1)) == 255) && (ord(substr($recordData, 13, 1)) == 255)) {
            ...
            } elseif ((ord(substr($recordData, 6, 1)) == 1)
                && (ord(substr($recordData, 12, 1)) == 255)
                && (ord(substr($recordData, 13, 1)) == 255)) {
                // Boolean formula. Result is in +2; 0=false, 1=true
                $dataType = PHPExcel_Cell_DataType::TYPE_BOOL;
                $value = (bool) ord(substr($recordData, 8, 1));
            } elseif ((ord(substr($recordData, 6, 1)) == 2)
                && (ord(substr($recordData, 12, 1)) == 255)
                && (ord(substr($recordData, 13, 1)) == 255)) {
                // Error formula. Error code is in +2
                $dataType = PHPExcel_Cell_DataType::TYPE_ERROR;
                $value = PHPExcel_Reader_Excel5_ErrorCode::lookup(ord(substr($recordData, 8, 1)));
            } elseif ((ord(substr($recordData, 6, 1)) == 3)
                && (ord(substr($recordData, 12, 1)) == 255)
                && (ord(substr($recordData, 13, 1)) == 255)) {
    ...
    private function readBoolErr()
    {
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            ...
            $boolErr = ord(substr($recordData, 6, 1));

            // offset: 7; size: 1; 0=boolean; 1=error
            $isError = ord(substr($recordData, 7, 1));
...
//        echo '<br />';
    ...
    private function readSelection()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            $paneId = ord(substr($recordData, 0, 1));
    ...
    private function readHyperLink()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            } elseif (ord(substr($recordData, $offset, 1)) == 0x03) {
                $hyperlinkType = 'local';
            } elseif (ord(substr($recordData, $offset, 1)) == 0xE0) {
                $hyperlinkType = 'URL';
            }
    ...
    private function readBIFF5CellRangeAddressFixed($subData)
    {
        ...
        $fc = ord(substr($subData, 4, 1));

        // offset: 5; size: 1; index to last column
        $lc = ord(substr($subData, 5, 1));
    ...
    private static function readRGB($rgb)
    {
        ...
        $r = ord(substr($rgb, 0, 1));

        // offset: 1; size: 1; Green component
        $g = ord(substr($rgb, 1, 1));

        // offset: 2; size: 1; Blue component
        $b = ord(substr($rgb, 2, 1));
