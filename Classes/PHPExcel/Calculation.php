...
class PHPExcel_Calculation
{
    ...
    {
        if (is_string($value)) {
            if ((isset($value) && strlen($value) > 0) && (substr($value, 0, 1) == '"') && (substr($value, -1) == '"')) {
...
//echo 'Assessing Expression '.substr($formula, $index), PHP_EOL;
            $opCharacter = substr($formula, $index, 1);//$formula{$index};    //    Get the first character of the value at the current index position
//echo 'Initial character of expression block is '.$opCharacter, PHP_EOL;
            if ((isset(self::$comparisonOperators[$opCharacter])) && (strlen($formula) > $index) && (isset(self::$comparisonOperators[substr($formula, $index+1, 1)]))) { //$formula{$index+1}
                $opCharacter .= substr($formula, ++$index, 1);//$formula{++$index};
...
//        }
    ...
    private function executeBinaryComparisonOperation($cellID, $operand1, $operand2, $operation, &$stack, $recursingArrays = false)
    {
        ...
        if (is_string($operand1) && $operand1 > '' && substr($operand1, 0, 1) == '"') {
            $operand1 = self::unwrapResult($operand1);
        }
        if (is_string($operand2) && $operand2 > '' && substr($operand2, 0, 1) == '"') {
