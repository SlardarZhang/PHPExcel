...
//        }
    ...
    private function cellToRowcol($cell)
    {
        ...
        for ($i = 0; $i < $col_ref_length; ++$i) {
            $col += (ord(mb_substr($col_ref, $i, 1)) - 64) * pow(26, $expn);
            --$expn;
        }
    ...
    private function advance()
    {
        ...
        // eat up white spaces
        if ($i < $formula_length) {
            while (mb_substr($this->formula, $i, 1) == " ") {
                ++$i;
            }

            if ($i < ($formula_length - 1)) {
                $this->lookAhead = mb_substr($this->formula, $i+1, 1);
            }
            $token = '';
        }

        while ($i < $formula_length) {
            $token .= mb_substr($this->formula, $i, 1);
            ...
            if ($i < ($formula_length - 2)) {
                $this->lookAhead = mb_substr($this->formula, $i+2, 1);
